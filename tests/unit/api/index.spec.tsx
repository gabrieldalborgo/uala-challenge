  ...
  describe('useFetchTransactions', () => {
    ...
    it('should filter transactions by card type', async () => {
      ...
      const { result } = renderHook(
        () => useFetchTransactions({ card: ['visa'] }),
        {
          wrapper: createWrapper(),
        }
      );
    ...
    it('should filter transactions by payment method', async () => {
      ...
      const { result } = renderHook(
        () => useFetchTransactions({ paymentMethod: ['link'] }),
        {
          wrapper: createWrapper(),
        }
      );
    ...
    it('should filter transactions by amount range', async () => {
      ...
      expect(result.current.data).toHaveLength(1);
      expect(result.current.data?.[0].amount).toBe(1500.5);
      expect(
        result.current.data?.every(t => t.amount >= 1000 && t.amount <= 2000)
      ).toBe(true);
    });
    ...
    it('should sort transactions by date (newest first)', async () => {
      ...
      const dates =
        result.current.data?.map(t => new Date(t.createdAt).getTime()) || [];
