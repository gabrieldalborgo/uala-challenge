  ...
  CardFilter: ({
    ...
    <div data-testid="card-filter">
      <button
        ...
        data-testid="card-filter-button"
      >
        Card Filter
      </button>
...
vi.mock('@/features/transactions/components/payment-method-filter', () => ({
  PaymentMethodFilter: ({
    ...
    <div data-testid="payment-method-filter">
      <button
        onClick={() => onChange(['qr'])}
        data-testid="payment-method-filter-button"
      >
        Payment Method Filter
      </button>
      <span data-testid="payment-method-filter-value">
        {value?.join(',') || 'none'}
      </span>
    </div>
  ),
...
vi.mock('@/features/transactions/components/installment-filter', () => ({
  InstallmentFilter: ({
    ...
    <div data-testid="installment-filter">
      <button
        onClick={() => onChange(['1'])}
        data-testid="installment-filter-button"
      >
        Installment Filter
      </button>
      <span data-testid="installment-filter-value">
        {value?.join(',') || 'none'}
      </span>
    </div>
  ),
...
vi.mock('@/features/transactions/components/amount-filter', () => ({
  AmountFilter: ({
    ...
    <div data-testid="amount-filter">
      <button
        onClick={() => onChange(100, 500)}
        data-testid="amount-filter-button"
      >
        Amount Filter
      </button>
...
describe('TransactionsFilters', () => {
  ...
  it('should clear all filters when clear button is clicked', () => {
    ...
    expect(screen.getByTestId('payment-method-filter-value')).toHaveTextContent(
      'none'
    );
  });
  ...
  it('should update current filters when props change', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('card-filter-value')).toHaveTextContent(
        'mastercard'
      );
    });
  });
  ...
  it('should pass current filter values to filter components', () => {
    ...
    expect(screen.getByTestId('date-filter-min')).toHaveTextContent(
      '2024-01-01'
    );
    expect(screen.getByTestId('date-filter-max')).toHaveTextContent(
      '2024-12-31'
    );
  });
});
