...
vi.mock('@/features/collections/components/tabs', () => ({
  Tabs: ({
    ...
    <div data-testid="tabs">
      <button onClick={() => setPeriodicity('daily')} data-testid="daily-tab">
        Diario
      </button>
      <button onClick={() => setPeriodicity('weekly')} data-testid="weekly-tab">
        Semanal
      </button>
      <button
...
vi.mock('@/features/collections/components/total', () => ({
  Total: ({ amount }: { amount: string }) => (
    <div data-testid="total">{amount}</div>
  ),
...
describe('CollectionsSummary', () => {
  const mockUseCollections = useCollections as MockedFunction<
  ...
  it('should initialize with weekly periodicity', () => {
    ...
    expect(screen.getByTestId('current-periodicity')).toHaveTextContent(
      'weekly'
    );
  });
  ...
  it('should update periodicity and call hook when tabs are clicked', () => {
    ...
    expect(mockUseCollectionsSpy).toHaveBeenCalledWith({
      periodicity: 'weekly',
    });
