...
test.describe('Transaction Filtering', () => {
  ...
  test('should filter transactions by Mastercard', async ({ page }) => {
    ...
    const initialTransactions = await page.locator('img[alt="Store icon"]').count()
    ...
    const filteredTransactions = await page.locator('img[alt="Store icon"]').count()
    expect(filteredTransactions).toBe(2)
    
    // Verify that the remaining transactions are Mastercard transactions
    const transactionElements = await page.locator('img[alt="Store icon"]').locator('..').locator('..').all()
