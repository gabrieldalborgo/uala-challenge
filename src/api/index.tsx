...
function useFetchApi<T>({
  select,
  queryKey,
  transform = (r: ResponseDto) => r,
}: UseFetchApiProps<T>): UseQueryResult<T> {
  ...
  return useFetchApi({
    ...
    transform: data => {
      ...
      if (filters.card && filters.card.length > 0) {
        response.transactions = response.transactions.filter(transaction =>
          filters.card!.includes(transaction.card)
        );
      }
      ...
      if (filters.installment && filters.installment.length > 0) {
        response.transactions = response.transactions.filter(transaction =>
          filters.installment!.includes(transaction.installments.toString())
        );
      }

      if (filters.minAmount) {
        response.transactions = response.transactions.filter(
