...
export function useTransactions({
  ...
  const {
    ...
    error: errorFromApi,
  } = useFetchTransactions(enhancedFilters);
  const {
    data: paymentMethods,
    isLoading: isLoadingPaymentMethods,
    error: errorFromPaymentMethods,
  } = useFetchPaymentMethods();
  const {
    data: cards,
    isLoading: isLoadingCards,
    error: errorFromCards,
  } = useFetchCards();

  const isLoading = useMemo(
    () => isLoadingTransactions || isLoadingPaymentMethods || isLoadingCards,
    [isLoadingTransactions, isLoadingPaymentMethods, isLoadingCards]
  );
  const error = useMemo(
    () => errorFromApi || errorFromPaymentMethods || errorFromCards,
    [errorFromApi, errorFromPaymentMethods, errorFromCards]
  );
  ...
  return {
    ...
    transactions:
      transactions.map(transaction => ({
        ...
        type:
          cards?.find(card => card.value === transaction.card)?.label ??
          'unknown',
      })) ?? [],
  };
}
